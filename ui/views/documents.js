...
import { api, requestFamilyDownload } from '../lib/api.js';
...
function uploadPanel(){
 ...
 return el('section',{class:'documents-panel documents-upload'},[
  el('h2',{text:'Choose a document'}),note('Statements, invoices and lab results. Text is extracted on the computer running Zelos, then sent to your configured AI on this computer or local network for review.'),
  ...
  el('div',{class:'documents-actions'},[button(view.busy==='reading'?'Reading document…':'Read document',{class:'btn solid',disabled:!!view.busy||!view.file,onClick:readDocument}),
...
function paint(){
 ...
 view.root.replaceChildren(...[
  ...
  view.busy && loadingState(view.busy==='reading'?'Reading your document…':view.busy==='saving'?'Saving reviewed records…':'Loading your document…', { layout: 'inline' }),
...
export async function useFamilyDocument(record){
 if(view.busy)throw new Error('Finish the document already being read before opening another.');
 if(!/\.(pdf|png|jpe?g)$/i.test(record?.filename||''))throw new Error('Imports can read PDF, PNG, and JPEG documents. Download this document to read it.');
 view.busy='loading';
 try{
  const {blob,filename}=await requestFamilyDownload(record.id);
  view.file=new File([blob],filename,{type:blob.type});view.preview=null;view.rows=[];view.receipt=null;view.error='';view.notice='Shared document loaded. Choose its type, then read and review it before importing.';invalidate();
 }finally{view.busy='';paint();}
}
