...
function paint() {
  ...
  if (cached.loading && data) content.push(loadingState('Refreshing your progress…', { layout: 'inline' }));
  ...
  if (!data) {
    content.push(pager);
    content.push(cached.loading ? loadingState('Loading your progress…', { layout: 'cards' }) : el('div', { class: 'progress-empty', role: 'status' }, [el('h2', { text: 'Progress is unavailable' }), note('Refresh to try again.')]));
