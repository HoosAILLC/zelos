...
function makeForm(title,fields,save,label='Save') {
  ...
  let saving=false;
  async function run(){
    if(saving||form.pending)return;saving=true;submit.disabled=true;
    ...
    try{const result=await save(form.values(),form);const saved=Object.values(result||{}).find(value=>value&&typeof value==='object'&&value.id);if(saved){form.id=saved.id;form.updatedAt=saved.updatedAt;}form.dirty=false;dirtyForms.delete(form);await load();status.textContent=result?.notice||'Saved in Zelos.';}
  ...
  form.submit=run;form.mark=mark;form.submitButton=submit;
  return form;
}
async function load(){
  if(loading)return;loading=true;
  message(data ? 'Refreshing health records…' : '');
  notice.classList.add('is-loading');
  if (!data) content.replaceChildren(loadingState('Loading health records…', { layout: 'cards' }));
  try{data=await api.healthTracking();paint();message('');}
  catch(error){message(error.message,true);if(!data)content.replaceChildren();}
  finally{loading=false;notice.classList.remove('is-loading');}
}
...
function walkingPanel(){
  ...
  let fileRequest=0;
  const cancelFileRead=()=>{fileRequest++;csv.pending=false;csv.submitButton.disabled=false;};
  csv.controls.csv.addEventListener('input',cancelFileRead);
  const clearCsv=csv.clear;csv.clear=()=>{cancelFileRead();file.value='';clearCsv();};
  file.addEventListener('change',async()=>{
    cancelFileRead();const revision=fileRequest,chosen=file.files?.[0];file.value='';
    if(!chosen)return;
    if(chosen.size>2000000){csv.status.textContent='Choose a CSV smaller than 2 MB.';return;}
    csv.pending=true;csv.submitButton.disabled=true;csv.status.textContent=`Reading ${chosen.name}…`;
    try{
      const contents=await chosen.text();if(revision!==fileRequest)return;
      csv.controls.csv.value=contents;csv.mark();csv.status.textContent=`Loaded ${chosen.name}. Review the rows, then choose Import CSV.`;
    }catch(error){if(revision===fileRequest)csv.status.textContent=`Could not read this file. ${error.message} Choose the file again or paste its CSV text.`;}
    finally{if(revision===fileRequest){csv.pending=false;csv.submitButton.disabled=false;}}
  });
...
function plannerPanel(){
  let preview=null,controller=null,modelWaiting=false,saving=false,saved=false,groceryManual=false,groceryReview=false,modelName='your configured model';
  ...
  const generate=button('Draft a week with your AI',{class:'btn solid',onClick:()=>run()});
  ...
  const node=el('section',{class:'health-form health-planner'},[
    el('div',{class:'health-planner-heading'},[el('h2',{text:'Plan a week with your AI'}),el('span',{class:'health-local-label',text:'Uses your saved preferences'})]),
  ...
  async function run(){
    ...
    const owned=new AbortController();controller=owned;modelWaiting=true;status.textContent='Your AI is drafting your week…';paint();
    try{
      ...
      modelWaiting=false;preview=JSON.parse(JSON.stringify(candidate));saved=false;groceryManual=false;groceryReview=false;renderPreview(result.model||'your configured model');
  ...
  async function commit(){
    ...
    saving=true;status.textContent='Saving your reviewed plan in Zelos…';paint();
    try{
      ...
      saved=true;status.textContent=`Saved your plan and ${result.groceryCount} grocery items in Zelos. No order was placed.`;
      await load();
