...
const rememberedKey='zelos.ask.active';
  ...
  try {
    ...
    for(const message of data.messages){
      if(message.role==='user'){item=exchange(message.content);transcript.appendChild(item.node);}
        ...
        if(message.state!=='complete')item.noteSlot.appendChild(el('p',{class:'exchange-note',text:message.state==='streaming'?'Zelos is still answering. This page will update automatically.':'This answer was interrupted. The saved text is shown above.'}));
...
function buildWebControls() {
  ...
  webSaveButton = button('Save search key', { class: 'btn quiet', onClick: async () => {
    ...
    try {
      ...
      webMessage(webConfigured ? 'Search key saved in Zelos. Enter a query, then choose Ask.' : 'The key was saved. Check search setup before trying again.');
...
async function ask(question, web = null, retryRevision = webRevision) {
  ...
  async function recover(){
    ...
    noteSlot.replaceChildren(el('p',{class:'exchange-note',text:'Reconnecting to the answer saved in Zelos…'}));
    const saved=await recoverAnswer({requestId,threadId,answerId,api,signal:mine.signal,onUpdate:message=>{
      ...
      answer.classList.remove('is-waiting','is-bad');rawAnswer=message.content||'';renderMarkdown(answer,rawAnswer||'Zelos is still thinking…');
      if(!rawAnswer && message.state==='streaming')answer.classList.add('is-waiting');
    ...
    noteSlot.replaceChildren(el('p',{class:'exchange-note',text:saved.state==='complete'?'Reconnected. Your saved answer is complete.':'The answer was interrupted. The saved text is shown above.'}));
  ...
  try {
    ...
    if (err?.name !== 'AbortError') {
      ...
      if(!gotAnswer)answer.textContent = err instanceof ApiError && err.status === 409 && /model.*configured|no.*model/i.test(err.message)
        ...
        : err.message;
      if(gotAnswer)noteSlot.replaceChildren(el('p',{class:'exchange-note',text:'Connection interrupted. Your saved answer is kept in Zelos.'}));
    ...
    if (stoppedByUser) {
      ...
      if (!await stopConfirmed) {
        ...
        noteSlot.appendChild(el('p',{class:'exchange-note',text:'Could not confirm Stop in Zelos. Reopen this conversation when connected to stop or recover the saved answer.'}));
      } else if (gotAnswer) {
...
function build() {
  ...
  field.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && (e.metaKey || e.ctrlKey) && !e.isComposing && !e.repeat && !e.altKey && !e.shiftKey) {
      e.preventDefault();
      form.requestSubmit();
    }
  });
