...
async function loadMessages({ more = false } = {}) {
  ...
  listStatus.classList.add('is-loading');
  if (!messages.length) {
    const placeholder = loadingState('Loading emails…');
    placeholder.querySelector('.zelos-loading-status').remove();
  ...
  try {
    ...
    if (listRequest === mine) {
      listRequest = null;
      listStatus.classList.remove('is-loading');
      list.querySelector('.zelos-loading')?.remove();
...
async function generate(editor) {
  ...
  try {
    ...
    status(editor, 'Your AI is drafting a reply…');
...
function makeEditor(message, detail) {
  ...
  editor.inputs = {
    ...
    body: el('textarea', { class: 'input mail-reply-body', rows: '9', 'aria-label': 'Reply body',
      placeholder: 'Write your reply, or ask your AI for a draft.' }),
    instructions: el('input', { class: 'input', type: 'text', 'aria-label': 'Draft instructions',
  ...
  accountOptions(editor.inputs.accountId, editor.accountId);
  editor.generate = button('Draft with AI', { class: 'btn quiet', onClick: () => generate(editor) });
...
function build() {
  ...
  pane = el('div', { class: 'mail-message-pane' }, el('div', { class: 'mail-empty' }, [
    el('h2', { text: 'Choose an email' }), el('p', { text: 'Read it here, write a reply, or ask your AI to prepare one.' }),
  ]));
