...
function paint(){
 ...
 if(!cache.settings){nodes.push(cache.error?note('Availability could not be loaded.'):loadingState('Loading your availability…'));if(cache.error)nodes.push(button('Retry availability',{class:'btn quiet',disabled:cache.loading,onClick:()=>load()}));cache.root.replaceChildren(...nodes);return;}
