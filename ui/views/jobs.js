...
async function load(){
  ...
  try{
    ...
    const focused=document.activeElement,focusedCard=focused?.closest?.('.job-card');
    const focusedId=focusedCard?.getAttribute('data-job-id'),focusedAction=focused?.getAttribute('data-job-action');
    const cards=data.jobs.map(job=>{
      const signature=JSON.stringify(job),previous=renderedJobs.get(job.id);
      if(previous?.signature===signature)return previous.node;
      const node=el('article',{class:'job-card','data-job-id':job.id},[
      el('div',{class:'job-card-top'},[el('h2',{text:job.prompt,tabindex:'-1'}),el('span',{class:`job-state is-${job.status}`,text:LABEL[job.status]||job.status})]),
      ...
      el('div',{class:'row-inline'},[
        ...(artifactsFor(job).map(artifact=>artifact.download===`/api/assistant/jobs/${job.id}/report.pdf`
          ?button('Download report',{class:'btn solid','data-job-action':'report',onClick:()=>downloadReport(job.id).catch(error=>{status.textContent=error.message;})})
          :typeof artifact.reviewUrl==='string'&&/^#\/mail\/draft\/[A-Za-z0-9_.:-]+$/.test(artifact.reviewUrl)?el('a',{class:'btn solid','data-job-action':'draft',href:artifact.reviewUrl,text:'Review email draft'}):null)),
        ['queued','running'].includes(job.status)?button('Stop task',{class:'btn quiet','data-job-action':'cancel',onClick:async()=>{try{await api.cancelJob(job.id);await load();}catch(error){status.textContent=error.message;}}}):null,
        ['needs_input','failed','completed','cancelled'].includes(job.status)?button('Assign follow-up',{class:'btn quiet','data-job-action':'follow-up',onClick:()=>{field.value=`Follow up on: ${job.prompt}\n\n`;field.focus();}}):null,
      ]),
      ]);
      if(previous?.node.querySelector('details')?.open&&node.querySelector('details'))node.querySelector('details').open=true;
      renderedJobs.set(job.id,{signature,node});return node;
    });
    if(cards.length!==list.children.length||cards.some((card,index)=>card!==list.children[index])) {
      list.replaceChildren(...cards);
      if(focusedId) {
        const card=renderedJobs.get(focusedId)?.node;
        if(focused?.isConnected)focusQuietly(focused);
        else if(card?.isConnected)focusQuietly(focusedAction?card.querySelector(`[data-job-action="${focusedAction}"]`)||card.querySelector('h2'):card.querySelector('summary')||card.querySelector('h2'));
      }
    }
    const ids=new Set(data.jobs.map(job=>job.id));for(const id of renderedJobs.keys())if(!ids.has(id))renderedJobs.delete(id);
    ...
    if(loadError){status.textContent='';loadError=false;}
  }catch(error){status.textContent=error.message;loadError=true;}finally{loading=false;}
}
export function renderJobs(){
  if(!root){
    ...
    buttonAssign=button('Assign to Zelos',{class:'btn solid',onClick:async()=>{
      ...
      try{await api.assignJob(prompt);if(field.value.trim()===prompt)field.value='';status.textContent='Your task is saved in Zelos. Keep Zelos running while it works.';await load();}
      catch(error){status.textContent=error.message;}finally{buttonAssign.disabled=false;}
    }});
    root=el('div',{class:'view view-jobs'},[
      el('h1',{text:'Assigned to Zelos'}),el('p',{class:'panel-lede',text:'Delegate work to your model and follow its progress. Tasks continue while Zelos and the computer running it stay on.'}),
