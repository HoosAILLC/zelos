...
import { createMealPlanner, defaultMealWeek } from '../lib/meal-planner.js';

const state = { root: null, data: null, selected: new Set(), title: 'My grocery list', review: null, busy: false, error: '', notice: '', loaded: false,
  loadRequest: 0, settingsDraft: null, settingsDirty: false, planner: null, weekStart: defaultMealWeek() };
...
async function load() {
  const request = ++state.loadRequest, requestedWeek = state.weekStart;
  state.busy = true; state.error = ''; paint();
  try {
    const data = await api.shopping(requestedWeek);
    if (request !== state.loadRequest || requestedWeek !== state.weekStart) return;
    const eligible = data.items.filter(item => item.state === 'needed').map(item => item.id);
  ...
  } catch (error) { if (request === state.loadRequest) state.error = error.message; }
  finally { if (request === state.loadRequest) { state.busy = false; paint(); } }
}
...
function budgetSummary(totals) {
  return el('div', { class: 'shopping-budget' }, [
    el('div', {}, [el('span', { text: 'Ingredient estimates' }), el('strong', { text: money(totals.estimatedMinor, totals.currency) })]),
    ...
    totals.status === 'over_estimate' ? el('p', { class: 'shopping-warning', text: 'Your ingredient estimates already exceed the weekly budget.' }) : null,
...
function setup() {
  ...
  const panel = disclosure('shopping-connection', state.data.setupIssues.length ? 'Connect a store' : 'Store and connection settings', [
    ...
    el('div', { class: 'shopping-actions' }, [
      button('Save connection and store', { class: 'btn quiet', disabled: state.busy, onClick: () => act(save, 'Grocery preferences saved in Zelos.') }),
...
function paint() {
  ...
  const nodes = [el('header', { class: 'shopping-heading' }, [el('div', {}, [el('h1', { text: 'Groceries' }), note('Choose your meals. Let Zelos handle the list.')]),
    el('div',{class:'workspace-actions'},[button(state.busy ? 'Working…' : 'Refresh', { class: 'btn quiet', disabled: state.busy, onClick: () => { state.review = null; load(); state.planner?.reload(); } }),el('a',{class:'btn solid',href:'#/health/groceries',text:'Add items'})])])];
  ...
  if (!data) { nodes.push(state.busy ? loadingState('Loading saved groceries…') : note('Refresh to load your grocery list.')); state.root.replaceChildren(...nodes); return; }
  if(state.review){nodes.push(reviewPanel());state.root.replaceChildren(...nodes.filter(Boolean));return;}
  if (data.mealPlanner) {
    if (!state.planner) state.planner = createMealPlanner({ initialView: 'discover', onWeekChange: weekStart => { state.weekStart = weekStart; state.review = null; state.loaded = false; load(); }, onListBuilt: async () => { await load(); state.selected = new Set(state.data.items.filter(i => i.state === 'needed').map(i => i.id)); paint(); } });
    nodes.push(state.planner.root);
  }
  if (data.groups.length || !data.mealPlanner) nodes.push(budgetSummary(data.totals));
  ...
  nodes.push(el('section', { class: 'shopping-panel' }, [
    ...
    ...(data.groups.length ? data.groups.map(groceryGroup) : [el('div', { class: 'shopping-empty' }, [el('h3', { text: 'Your grocery list is empty' }), note('Choose meals above to build your list, or add your own items.')])]),
    el('div', { class: 'shopping-actions' }, [
...
export function renderShopping() {
  ...
  else if (!state.root.isConnected) { state.review = null; state.planner?.reload(); if (!state.busy) load(); }
  return state.root;
}
