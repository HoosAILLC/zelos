...
3. Tag that exact commit `v<version>` and push the tag. The desktop workflow runs the suite, builds both Mac and Windows architectures, checks the versions, and publishes a GitHub release only after all four builds succeed. The release includes the exact source archive, checksums, and a manifest with the commit.
...
Website-only updates use `website/release.json` to keep downloads on a verified published release. The app's package version names the newer feature preview and demo separately. After a new release is published, refresh this pinned file from that release's official `release.json`, checking the source commit and asset checksums. Tagged builds use the matching manifest prepared in `release-assets/release.json` instead. Do not point regular download buttons at an unpublished version or temporary QA artifacts.
