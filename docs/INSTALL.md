...
```
Zelos-1.1.3-arm64.dmg          Apple Silicon
Zelos-1.1.3-x64.dmg            Intel Macs
Zelos-1.1.3-setup-x64.exe      Windows, 64-bit — ordinary PCs
Zelos-1.1.3-setup-arm64.exe    Windows on ARM
```
...
1. **Take the installer that matches your machine.**
   `Zelos-1.1.3-setup-x64.exe` for an ordinary PC, `-arm64` for Windows on ARM,
...
```
Unblock-File .\Zelos-1.1.3-setup-x64.exe
```
