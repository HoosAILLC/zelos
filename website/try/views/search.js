...
export function destinationFor(ref, { items = [], events = [] } = {}) {
  ...
  if (parts.prefix === 'msg') return { where: 'mail', hash: '#/mail/' + encodeURIComponent(parts.id) };
...
function metaLine(dest) {
  if (!dest || dest.where === 'mail') return null;
...
function openControl(dest) {
  if (dest.where === 'mail') return button('Open email', { class: 'btn quiet', onClick: () => navigateTo(dest.hash) });
  if (dest.where === 'calendar') {
