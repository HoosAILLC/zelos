...
const SECRET_SHAPES = [
  ...
  /\bzf[saic]_[A-Za-z0-9_-]{43}/g,        // Family sessions, API credentials, invitations, and MFA challenges
  /\bzfr_[A-Za-z0-9_-]{22}/g,             // One-use family recovery codes
  /otpauth:\/\/[^\s<>"'\\]+/gi,           // Authenticator enrollment URIs contain a seed
...
const SECRET_KEYS = new Set([
  ...
  'currentpassword', 'otp', 'totp', 'mfacode', 'recoverycode', 'recoverycodes', 'challenge', 'otpauthuri',
]);
...
const SECRET_ASSIGNMENT = /((?:^|[^\w])['"]?(?:pass(?:word|wd)?|current[-_ ]?password|app[-_ ]?password|api[-_ ]?key|access[-_ ]?token|refresh[-_ ]?token|client[-_ ]?secret|device[-_ ]?code|session[-_ ]?token|recovery[-_ ]?codes?|mfa[-_ ]?code|otp|totp|challenge|otpauthuri|x-api-key|authorization|credentials|secret|token)['"]?\s*[:=]\s*)("(?:\\.|[^"\\])*"?|'(?:\\.|[^'\\])*'?|[^\s,;&}\]]+)/gi;
