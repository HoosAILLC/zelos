...
test('a collected local OpenAI completion survives long reasoning without another request', async () => {
  const frame = (event) => `data: ${JSON.stringify(event)}\n\n`;
  const reply = '{"first":null,"items":[],"notes":["A quiet morning."]}';
  mock.plan.push({
    chunks: [
      frame({ model: 'local-reasoning-model', choices: [{ delta: { role: 'assistant' } }] }),
      frame({ choices: [{ delta: { reasoning: 'private synthetic reasoning' } }] }),
      ': proxy keepalive\n\n',
      frame({ choices: [{ delta: { reasoning: 'more synthetic reasoning' } }] }),
      frame({ choices: [{ delta: { reasoning: 'still progressing' } }] }),
      frame({ choices: [{ delta: { content: reply.slice(0, 24) } }] }),
      frame({ choices: [{ delta: { content: reply.slice(24) } }] }),
      frame({ choices: [{ delta: {}, finish_reason: 'stop' }], usage: { prompt_tokens: 19620, completion_tokens: 12000 } }),
      'data: [DONE]\n\n',
    ],
    delayMs: 150,
  });
  const started = Date.now();
  const result = await complete({
    protocol: 'openai', baseUrl: mock.origin, model: 'local-reasoning-model',
    messages: [{ role: 'user', content: 'Return a board.' }],
    json: true, stream: true, maxTokens: 16384, timeoutMs: 700, retries: 0,
  });
  assert.ok(Date.now() - started > 700, 'reasoning outlasted the former total deadline');
  assert.equal(mock.requests.length, 1, 'an active generation must not be repeated');
  assert.equal(mock.last().body.stream, true);
  assert.equal(mock.last().body.max_tokens, 16384);
  assert.equal(result.text, reply, 'reasoning never enters the board');
  assert.deepEqual(result.usage, { input: 19620, output: 12000 });
  assert.equal(result.stopReason, 'stop');
});
