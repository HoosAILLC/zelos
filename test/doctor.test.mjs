...
describe('the database check', () => {
  ...
  test('a corrupt zelos.db is a failure that says to move it aside', async () => {
    ...
    assert.match(db.action, /renaming .*zelos\.db.*zelos\.db\.broken/, 'the remedy identifies the file on every platform');
    assert.match(db.action, /finance, health, document and board records/, 'starting fresh must disclose which saved records become unavailable');
    assert.match(db.action, /copy its data folder.*verified backup/, 'recovery guidance must preserve data before starting fresh');
