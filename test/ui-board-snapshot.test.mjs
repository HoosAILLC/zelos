...
test('board snapshot downloads board content while excluding settings, diagnostics and raw event imports', async t => {
  ...
  assert.deepEqual(calls.map(call => call.path), ['/api/data', '/api/backups/automatic', '/api/state'], 'panel reads backup status; snapshot does not request connection settings');
